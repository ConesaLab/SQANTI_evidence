...
## [2026-09-17] - Rescue-Path Filter as the Default, Noise Diagnostic, Named Tier Attribute (v1.1.0 code, release checklist A1, A3, A4; A2 dropped)
- **Branch**: `dev-IsoSeq_v2`
- **Goal**: The filter-design experiment (paper repo, `filter_design_rescue_paths.md` §1–17) showed that the single six-rule filter removes real isoforms mainly through the intra-priming check, and that letting an isoform through when it fails exactly one of the intra-priming, psauron or canonical-junction checks but carries ≥10 full-length reads ties or improves both F1 scores in 8/8 benchmark species (best arm outright in worm and zebrafish), adds 1–26 % reference-exact genes to Tier 1, and never lowers annotation BUSCO. Decision (Pablo, 16 Sep): this rule becomes the default. Because the rescue paths admit more junk on noisy transcriptomes (zebrafish: +973 `u`/`i` for +891 exact), the pipeline should tell the user how noisy the transcriptome is before it filters.
- **Summary**:
    - `envs/filter_rules.json`: now four OR-ed paths — the strict rule, and the strict rule with one of intra-priming / psauron / canonical waived when `FL >= 10`. Byte-identical (sorted JSON) to the `designed_rules.json` the eight benchmark arms ran with. The previous single rule is kept as `envs/filter_rules.strict.json` (the v1.0 filter, the ablation baseline).
    - New `scripts/noise_metrics.py` + `rule noise_diagnostic` (`transcript_modelling.smk`, before `filter_isoforms`, which declares the table as an input so it always runs): from the SQANTI3 classification alone it computes the non-coding, FL≤2, mono-exon and ≥3-strict-rules-failed fractions, writes `quality_control/noise_metrics.tsv` and logs a verdict (clean < 20 % non-coding < moderate < 40 % < noisy) on the benchmark scale (Arabidopsis 2.6 % … zebrafish 60.8 %), with the advice to raise the `FL` value of the rescue paths or use the strict file when noisy. Non-coding fraction was chosen because it had rho 0.88 with the reference-based junk fraction across the eight species.
    - README: `filter_rules` entry rewritten (four paths, how to tighten, strict file), curation step describes the diagnostic, output tree lists `noise_metrics.tsv`, `genome` entry now states soft-masking is expected with the ERGA numbers.
    - Tests: `tests/test_noise_metrics.py` (5).
    - `scripts/resolve_transcript_tiers.py`: new `add_tier()` applied at write time appends `tier "SQANTI_curated";` to every Tier 1 row and `tier "Augustus";` to every Tier 2 row. Augustus `gene`/`transcript` rows, which carry a bare id in column 9, are rewritten as `gene_id "g"; tier ...` / `transcript_id "g.t1"; gene_id "g"; tier ...` so the tag is present at every feature level (checked with AGAT on Garnatxa: `tier=` now appears on gene, transcript, exon, CDS and UTR rows of both tiers in the final GFF3; without the rewrite the Tier 2 gene/transcript rows lacked it). Names rather than numbers by Pablo's choice. `tests/test_resolve_transcript_tiers.py` +2.
    - Considered and dropped (Pablo, 17 Sep): a `curation.rescue_min_fl` key that rewrote the `FL` values of the rules JSON at run time. A user-supplied SQANTI3 rules file must run exactly as written; tuning the threshold is a copy-and-edit of three values, documented in README, and the strict file covers the no-rescue case.
    - 111 tests and 8/8 dry-run configurations pass; every DAG gains one job (`noise_diagnostic`).
...
- **Summary**:
    - New `scripts/restore_gene_ids.py` and rule `restore_gene_ids` (after `filter_isoforms`): maps each surviving `transcript_id` to its IsoQuant `gene_id` and rewrites `gene_id` in the GTF and `associated_gene` in the RulesFilter classification (`*.filtered.regrouped.gtf`, `*_RulesFilter_classification.regrouped.txt`); transcripts absent from the models keep their SQANTI id and are logged.
    - `select_dominant_sqanti_genes`, `extract_rna_hints` and `resolve_transcript_tiers` read the regrouped files.
    - Tests `tests/test_restore_gene_ids.py` (5); README output tree updated. GffCompare metrics unaffected (coordinate based); gene counts and the dominant-isoform set change. Measured on finished runs: human 1.88, zebrafish 1.33, Drosophila 1.29, Arabidopsis 1.08 transcripts/gene.
