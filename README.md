...
| `reconstruction` | Which tool builds the transcriptome from the reads. `isoquant` (default) runs IsoQuant. `isoseq` runs the PacBio Iso-Seq route instead: `isoseq cluster2` → `pbmm2 align` → `isoseq collapse`. Both produce the same two files (transcript models plus a full-length count matrix), so everything downstream is identical. The `isoseq` route is PacBio-only and needs an unmapped PacBio BAM; a FASTA/FASTQ input is wrapped in one using a template (`envs/pacbio_mock.bam`), which preserves read names, sequences and qualities but fakes the run metadata — the pipeline warns, and you should confirm the reads really are CCS/HiFi first. |
