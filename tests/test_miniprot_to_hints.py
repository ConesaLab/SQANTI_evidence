...
def test_miniprot_to_hints_positive_strand():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert "chr1\tminiprot\tCDSpart\t1000\t1200\t.\t+\t0\tgrp=prot_gene1;pri=2;src=P" in lines
        assert "chr1\tminiprot\tCDSpart\t1800\t2200\t.\t+\t1\tgrp=prot_gene1;pri=2;src=P" in lines
        assert "chr1\tminiprot\tCDSpart\t2800\t3000\t.\t+\t2\tgrp=prot_gene1;pri=2;src=P" in lines

        # Check introns
        assert "chr1\tminiprot\tintron\t1201\t1799\t.\t+\t.\tgrp=prot_gene1;pri=2;src=P" in lines
        assert "chr1\tminiprot\tintron\t2201\t2799\t.\t+\t.\tgrp=prot_gene1;pri=2;src=P" in lines

        # Check start & stop on + strand
        assert "chr1\tminiprot\tstart\t1000\t1002\t.\t+\t0\tgrp=prot_gene1;pri=3;src=P" in lines
        assert "chr1\tminiprot\tstop\t2998\t3000\t.\t+\t0\tgrp=prot_gene1;pri=3;src=P" in lines
...
def test_miniprot_to_hints_negative_strand():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Check intron
        assert "chr2\tminiprot\tintron\t5501\t6499\t.\t-\t.\tgrp=prot_gene2;pri=2;src=P" in lines

        # On '-' strand: start is at highest coordinate (6998..7000), stop is at lowest coordinate (5000..5002)
        assert "chr2\tminiprot\tstart\t6998\t7000\t.\t-\t0\tgrp=prot_gene2;pri=3;src=P" in lines
        assert "chr2\tminiprot\tstop\t5000\t5002\t.\t-\t0\tgrp=prot_gene2;pri=3;src=P" in lines
